...
export class LedgerService {
  ...
  private async account(organizationId: string, code: string) {
  ...
  /** On PAID: credit PENDING (not AVAILABLE) until settlement */
  async postSale(paymentId: string) {
    ...
    const gatewayFeeBps = Number(this.config.get('GATEWAY_FEE_BPS') || 70); // ~0.7% Midtrans-like
  ...
  /** Move net from PENDING → AVAILABLE after settlement window */
  async settlePending(olderThanHours = 24) {
    ...
    const entries = await this.prisma.merchantLedgerEntry.findMany({
      where: {
        ...
        meta: { equals: undefined as any },
      },
      take: 100,
    });

    // Find payments settled via sale gross without SETTLE entry
  ...
  async postRefund(paymentId: string, amount: number, refundId: string) {
    ...
    // Prefer debit available, then pending
  ...
  async adjust(organizationId: string, amount: number, reason: string, actorId: string) {
