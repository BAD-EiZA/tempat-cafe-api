import { Injectable, Logger, OnModuleDestroy, OnModuleInit, Inject, forwardRef, Optional } from '@nestjs/common';
...
export class OutboxWorker implements OnModuleInit, OnModuleDestroy {
  ...
  private async reservationReminders() {
    ...
    for (const r of list) {
      ...
      // in-app notify branch managers (best-effort)
      ...
      for (const m of members) {
        await this.notifications.create(
          ...
          `Reservasi ${r.customerName}`,
          `Check-in ~${new Date(r.startAt).toLocaleString('id-ID')}`,
          { reservationId: r.id },
        );
      }
    }
  ...
  private async handle(event: {
    ...
    let branchId = payload.branchId as string | undefined;
    const orderId = (payload.orderId || event.aggregateId) as string;
